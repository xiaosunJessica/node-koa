  ...
  methods: {
    ...
    watchDeployStatus (deployId) {
      ...
      if (this.serverLogEvent === null || this.serverLogEvent.readyState === this.eventSourceState.CLOSED) {
        this.serverLogEvent = new EventSource('backend/streams/' + deployId, {
    ...
    async getDeployResult (deployId) {
      try {
        ...
        if (res.status === 'successful') {
          ...
          console.log(appInfo, 'appInfoappInfoappInfoappInfo')
    ...
    async getPreDeployDetail () {
      try {
        ...
        console.log(timeLineList, ' this.timeLineList  this.timeLineList ')
    ...
    watchServerPush () {
      const url = `backend/svc_workloads/api/processes/applications/${this.appCode}/modules/${this.curModuleId}/envs/${this.environment}/processes/watch/?rv_proc=${this.prevProcessVersion}&rv_inst=${this.prevInstanceVersion}&timeout_seconds=${this.serverTimeout}`
    ...
    formatDisplayBlock (displays) {
      ...
      console.log(this.curAppModule.application, '--keysss-')
      for (const key of keys) {
        ...
        if (this.curAppInfo.application && this.curAppInfo.application.type === 'bk_plugin') {
          sourceInfo.push({
            text: this.$t('模块类型'),
            value: this.$t('蓝鲸插件'),
        ...
        // 普通应用不展示
        if (this.curAppModule.web_config && this.curAppModule.web_config.templated_source_enabled && this.curAppModule.source_origin !== this.GLOBAL.APP_TYPES.NORMAL_APP) {
          sourceInfo.push({
            text: this.$t('初始化模板类型'),
        ...
        if (this.curAppModule.web_config && this.curAppModule.web_config.runtime_type !== 'custom_image') {
          const smartRoute = [
            {
              name: this.$t('查看包版本'),
              route: {
        ...
        switch (key) {
          ...
          case 'source_info':
            displayBlocks.push({
              name: this.curAppModule.web_config && this.curAppModule.web_config.runtime_type === 'custom_image' ? this.$t('镜像信息') : this.$t('源码信息'),
              type: 'key-value',
    ...
    handleDownloadTemplate () {
      const url = `backend/api/bkapps/applications/${this.appCode}/modules/${this.curModuleId}/sourcectl/init_template/`
